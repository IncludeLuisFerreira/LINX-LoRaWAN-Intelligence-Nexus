# Guia de Reprodução — Infraestrutura LoRaWAN (Fase Inicial)

> Este documento preserva o guia completo de como a infraestrutura LoRaWAN foi montada do zero na fase inicial do projeto, antes da migração para a arquitetura LINX. Serve como referência para reproduzir o ambiente ou entender decisões técnicas tomadas.
...
O arquivo já contém as definições para AU915 na seção `#ifdef AU915_928`. Verifique se as frequências estão corretas:
...
> Após o upload, abra o Serial Monitor (115200 baud). O gateway exibirá "Connecting to WiFi...", o IP obtido, e em seguida "CAD start". Acesse o IP no navegador para o painel web.
...
void configura_camada_rede() {
    // OTAA: sessão derivada do Join Accept — nenhuma sessão fixa aqui.
}
```
...
void APP_radio_send_UL(){
    ...
    Pacote_UL[0xD] = (ldr >> 8) & 0xFF;
    Pacote_UL[0xE] = ldr & 0xFF;
}
```
...
2. **Devices → Add**
3. Device Profile: selecione o criado no passo 1.4
4. **Activation**: selecione **OTAA**
5. Preencha DevEUI, AppEUI/JoinEUI e AppKey com os mesmos valores de `nvs_secrets.h`
6. **Save**

> Em OTAA **não** há DevAddr/NwkSKey/AppSKey fixos no ChirpStack — eles são derivados no momento do join. Deixe **Disable frame-counter validation** desmarcado.
...
4. **Upload** (CTRL+U) e abra o Serial Monitor (115200 baud)
