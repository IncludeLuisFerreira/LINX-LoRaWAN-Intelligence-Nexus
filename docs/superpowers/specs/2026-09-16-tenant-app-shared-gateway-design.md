# Design: tenant_app_template consumindo o gateway compartilhado
...
- não existe configuração do endereço do gateway compartilhado.

## Objetivo

Representar corretamente o ambiente tenant no template: subir `tenant_app` +
`timescaledb`, com o tenant app configurado para falar com o **gateway
compartilhado** via `CLIENT_AGENT_URL`. A implementação do cliente HTTP
tenant→gateway fica para issue futura.

## Não-objetivos

- Implementar o cliente HTTP do tenant app para o gateway.
- Implementar proxy REST/WebSocket, gRPC server do gateway, JWT/RBAC.
...
- O gateway é **externo** ao compose (compartilhado), então não há `depends_on`
  nem serviço de gateway aqui.

### `tenant_app_template/.env.example`

| Variável            | Valor                     | Descrição                                      |
| ------------------- | ------------------------- | ---------------------------------------------- |
| `CLIENT_AGENT_URL`  | `http://localhost:8001`   | URL base do gateway compartilhado.             |
| `APP_ID`            | `app-abc123`              | Identidade da aplicação.                       |
| `MQTT_TOPIC`        | `au915_0/gateway/+/event/up` | Tópico MQTT de uplink.                     |
| `TENANT_PORT`       | `8002`                    | Porta exposta do tenant app (evita colisão com o gateway 8001). |
...
- Reescreve a seção "Docker Compose (ambiente tenant completo)": sobe
  `timescaledb` + `tenant_app`; o tenant app consome o gateway compartilhado.
- Remove referências ao `client_agent` por-tenant.
- Atualiza a tabela de variáveis com `CLIENT_AGENT_URL` e `TENANT_PORT=8002`.
- Nota: o cliente HTTP tenant→gateway é issue futura; aqui entra apenas a
