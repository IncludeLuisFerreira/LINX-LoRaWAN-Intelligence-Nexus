...
// ============================================================================
// IMPLEMENTAÇÃO MANUAL DAS FUNÇÕES BASE64 (Correção para novos Cores ESP32)
// ============================================================================
...
int base64_dec_len(const char * src, int srcLen) {
    int i = srcLen - 1;
    int count = 0;
    ...
    return ((srcLen * 3) / 4) - count;
}

int base64_encode(char * dst, const char * src, int srcLen) {
    int i, j;
    int encLen = 0;
    unsigned char a3[3];
    unsigned char a4[4];
    ...
    while (srcLen--) {
        ...
        if (i == 3) {
            ...
            a4[3] = a3[2] & 0x3f;
            for (i = 0; i < 4; i++) dst[encLen++] = b64_chars[a4[i]];
            i = 0;
        }
    }
    ...
    if (i) {
        ...
        a4[2] = ((a3[1] & 0x0f) << 2) + ((a3[2] & 0xc0) >> 6);
        a4[3] = a3[2] & 0x3f;
        for (j = 0; j < i + 1; j++) dst[encLen++] = b64_chars[a4[j]];
        while (i++ < 3) dst[encLen++] = '=';
    }
    dst[encLen] = '\0';
    return encLen;
}
...
int base64_decode(char * dst, const char * src, int srcLen) {
    ...
    if (i) {
        ...
        a3[1] = ((a4[1] & 0x0f) << 4) + ((a4[2] & 0x3c) >> 2);
        a3[2] = ((a4[2] & 0x03) << 6) + a4[3];
        for (j = 0; j < i - 1; j++) dst[decLen++] = a3[j];
    }
    dst[decLen] = '\0';
    return decLen;
}
...
int buildPacket(uint8_t *buff_up, struct LoraUp *LoraUp, bool internal) 
{
	...
	char cfreq[9] = {0};								// Character array to hold freq in MHz
...
// ---------------------------------------
	...
	//strcpy(cfreq, String(freqs[gwayConfig.ch].upFreq / 100000).c_str());
	strcpy(cfreq, "916.800000");
